				...
				void *data)
{
	...
	if (name) {
		trace_printk("[DEBUG5] vfs_kern_mount\n");
			...
			int mnt_flags, const char *name, void *data)
{
	...
	if (subtype) {
		trace_printk("[DEBUG5] do_new_mount1\n");
		err = vfs_parse_fs_string(fc, "subtype",
					  subtype, strlen(subtype));
	}
	if (!err && name) {
		trace_printk("[DEBUG5] do_new_mount2\n");
		err = vfs_parse_fs_string(fc, "source", name, strlen(name));
	}
	if (!err) {
		trace_printk("[DEBUG5] do_new_mount3\n");
		err = parse_monolithic_mount_data(fc, data);
	}
	if (!err && !mount_capable(fc)) {
		trace_printk("[DEBUG5] do_new_mount4\n");
		err = -EPERM;
	}
	if (!err) {
		trace_printk("[DEBUG5] do_new_mount5\n");
		err = vfs_get_tree(fc);
	}
	if (!err) {
		trace_printk("[DEBUG5] do_new_mount5\n");
		...
		const char *type_page, unsigned long flags, void *data_page)
{
	...
	data1 = data_page;
	trace_printk("[DEBUG5] path_mount data_page: %s\n", data1);
		...
		const char *type_page, unsigned long flags, void *data_page)
{
	...
	if (ret)
		return ret;
	data1 = data_page;
	trace_printk("[DEBUG5] do_mount data: %s\n", data1);
		...
		char __user *, type, unsigned long, flags, void __user *, data)
{
	...
	data1 = options;
	trace_printk("[DEBUG5] SYSCALL_DEFINE5 data: %s\n", data1);
