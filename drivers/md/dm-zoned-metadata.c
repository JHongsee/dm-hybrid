...
unsigned int dmz_nr_unmap_chunk_cache_zones(struct dmz_metadata *zmd)
{
	struct list_head *unmap_zone_list = &zmd->unmap_cache_list;
	struct list_head *map_zone_list = &zmd->map_cache_list;
	struct dm_zone *zone;
	unsigned int nr_cache = dmz_nr_cache_zones(zmd);
	unsigned int nr_avail = nr_cache * DMZ_CHUNK_PER_RZ;
				    ...
				    struct dm_zone *zone)
{
	...
	if (zone->wp_block < wp) {
		trace_printk("[DEBUGINV] dmz_handle_seq_write_err dmz_invalidate_blocks start zone: %u \n", zone->id);
		dmz_invalidate_blocks(zmd, zone, zone->wp_block,
				      wp - zone->wp_block);
		trace_printk("[DEBUGINV] dmz_handle_seq_write_err dmz_invalidate_blocks end\n");
	}
...
static void __dmz_lru_zone(struct dmz_metadata *zmd, struct dm_zone *zone)
{
	...
	if (list_empty(&zone->link)) {
		trace_printk("[DEBUG3] __dmz_lru_zone list_empty error\n");
	}
	

	if (zone == NULL) {
		trace_printk("[DEBUG3] __dmz_lru_zone zone null error\n");
	}
 ...
 */
static void dmz_lru_zone(struct dmz_metadata *zmd, struct dm_zone *zone/*modi*/, unsigned int chunk_id)
{
	if (zone == NULL) { trace_printk("[DEBUG3] dmz_lru_zone zone == NULL error\n"); }
	...
	if (zone == NULL) { trace_printk("[DEBUG3] dmz_lru_zone zone == NULL error\n"); }
	if (&zone->chunks == NULL) { trace_printk("[DEBUG3] dmz_lru_zone zone->chunks == NULL error\n"); }
	if (list_empty(&zone->chunks)) { trace_printk("[DEBUG3] dmz_lru_zone list_empty(&zone->chunks) error\n"); }
	list_for_each_entry(c, &(zone->chunks), link) {
		if (c->id == chunk_id) { break; }
	}
	if (c == NULL) { trace_printk("[DEBUG3] dmz_lru_zone c == NULL error\n"); }
	if (c->id != chunk_id) { trace_printk("[DEBUG] dmz_lru_zone c->id != chunk_id error\n"); }
	if (c->szone) {
		__dmz_lru_zone(zmd, c->szone);
	}
	/*modi*/
}

/* chunk list modi */
static void dmz_lru_chunk(struct dmz_metadata *zmd, struct dm_chunk *c)
{
	if (list_empty(&zmd->mapped_chunk_list)) 
		return;

	list_del_init(&c->map_link);
	list_add_tail(&c->map_link, &zmd->mapped_chunk_list);
}
/* chunk list modi */
...
int dmz_lock_chunk_reclaim(struct dm_chunk *chunk)
{
	...
	return !test_and_set_bit(DMZ_CHUNK_RECLAIM, &chunk->flags);
//	return 1;
}
/* chunk list modi */
...
/* chunk list modi */
void dmz_unlock_chunk_reclaim(struct dm_chunk *chunk)
{
//	WARN_ON(dmz_is_active(zone));
//	WARN_ON(!dmz_in_reclaim(zone));

	clear_bit_unlock(DMZ_CHUNK_RECLAIM, &chunk->flags);
	smp_mb__after_atomic();
	wake_up_bit(&chunk->flags, DMZ_CHUNK_RECLAIM);
}
/* chunk list modi */

/* chunk IO block modi */
void dmz_unlock_chunk_for_zone_reclaim(struct dm_chunk *chunk)
{
//	WARN_ON(dmz_is_active(zone));
//	WARN_ON(!dmz_in_reclaim(zone));
...
//	wake_up_bit(&chunk->flags, DMZ_CHUNK_ZONE_RECLAIM);
	...
	smp_mb__after_atomic();
	wake_up_bit(&chunk->flags, DMZ_CHUNK_ZONE_RECLAIM);
	trace_printk("[WAIT] chunk %u wait end\n", chunk->id);
...
/* chunk list modi */
static void dmz_wait_chunk_for_reclaim(struct dmz_metadata *zmd, struct dm_chunk *chunk)
{
	dmz_unlock_map(zmd);
	dmz_unlock_metadata(zmd);
	set_bit(DMZ_CHUNK_RECLAIM_TERMINATE, &chunk->flags);
	wait_on_bit_timeout(&chunk->flags, DMZ_CHUNK_RECLAIM, TASK_UNINTERRUPTIBLE, HZ);
	clear_bit(DMZ_CHUNK_RECLAIM_TERMINATE, &chunk->flags);
	dmz_lock_metadata(zmd);
	dmz_lock_map(zmd);
}
/* chunk list modi */
						    ...
						    unsigned int idx, bool idle)
{
	...
	/*modi*/
	if (zone_list == NULL) {
		trace_printk("\t\t\t[DEBUGRECL] dmz_get_rnd_zone_for_reclaim zone_list == NULL error\n");
	}
	if (list_empty(zone_list)) {
		trace_printk("\t\t\t[DEBUGRECL] dmz_get_rnd_zone_for_reclaim zone_list empty error\n");
		return NULL;
	}
...
/* chunk list modi */
static struct dm_chunk *dmz_get_chunk_for_reclaim_in(struct dmz_metadata *zmd,
						    unsigned int idx, bool idle)
{
	struct dm_chunk *chunk = NULL;
	struct dm_chunk *maxw_c = NULL;
	struct list_head *chunk_list = &zmd->mapped_chunk_list;
...
/* chunk list modi */
struct dm_chunk *dmz_get_chunk_for_reclaim(struct dmz_metadata *zmd,
					 unsigned int dev_idx, bool idle)
{
	struct dm_chunk *chunk = NULL;
	...
	dmz_lock_map(zmd);
//	zone = dmz_get_rnd_zone_for_reclaim(zmd, dev_idx, idle);
	chunk = dmz_get_chunk_for_reclaim_in(zmd, dev_idx, idle);
	dmz_unlock_map(zmd);
...
/* zone reclaim modi */
bool dmz_get_high_weight_zone_for_reclaim(struct dmz_metadata *zmd, 
				unsigned int idx, bool idle, struct dm_zone *zone, struct dm_chunk **ret_c) {
//    struct dm_zone *zone;
    struct dm_chunk *c, *maxw_c = NULL;
	struct list_head *zone_list = NULL;
    dmz_lock_map(zmd);
	/*
    if (zmd->nr_cache) {
        zone_list = &zmd->map_cache_list;
	...
	if (zone_list == NULL) {
		trace_printk("[DEBUGRAC] dmz_get_high_weight_zone_for_reclaim list empty\n");
	}
	*/
	/*
    list_for_each_entry(zone, zone_list, link) {
        if (zone->weight * 100 / DMZ_BLOCK_PER_ZONE > DMZ_ZONE_RECLAIM_WEIGHT) {
			if (&zone->chunks == NULL) {
				trace_printk("[DEBUGRAC] dmz_get_high_weight_zone_for_reclaim chunk list empty\n");
			}
			*/
            list_for_each_entry(c, &zone->chunks, link) {
			...
			if (dmz_lock_chunk_reclaim(maxw_c)) {
	            *ret_c = maxw_c;
//	            *ret_zone = zone;
	            dmz_unlock_map(zmd);
				return true;
...
struct dm_chunk* dmz_get_chunk_snapshot(struct dmz_metadata *zmd, struct dm_chunk *c) {
	...
	trace_printk("[WC] dmz_get_chunk_snapshot start\n");
	while (i < DMZ_BLOCK_PER_ZONE) {
		if (c->write_check[i] == c->id) {
			trace_printk("[WC] c %u i %d c->offsets[i] %d\n", c->id, i, c->offsets[i]);
			snap->offsets[i] = c->offsets[i];
			snap->write_check[i] = 1;
			c->offsets[i] = -1;
			c->write_check[i] = -1;
			trace_printk("[WC] c %u i %d, c->offsets[i] %d set\n", c->id, i, c->offsets[i]);
//			trace_printk("[WC] c %u i %d, snap->offsets[i] %d\n", c->id, i, snap->offsets[i]);
			snap->weight++;
		}
		else {
			snap->offsets[i] = -1;
	...
	c->weight -= snap->weight;
	snap->szone = c->szone;
//	snap->rz_weight = c->rz_weight;
...
*/	
	...
	trace_printk("[WC] dmz_get_chunk_snapshot end\n");
	return snap;
}


bool dmz_get_high_weight_c(struct dmz_metadata *zmd, 
				unsigned int idx, bool idle, struct dm_zone *zone, struct dm_chunk **ret_c) {
    struct dm_chunk *c = *ret_c;
    dmz_lock_map(zmd);
	...
	*ret_c = dmz_get_chunk_snapshot(zmd, c);
    dmz_unlock_map(zmd);
	return true;
...
again:
	...
	if (dzone_id == DMZ_MAP_UNMAPPED) {
		...
		dzone = dmz_alloc_zone(zmd, 0, alloc_flags/*, nr_blocks*/);
		if (!dzone) {
			trace_printk("[DEBUG] dmz_get_chunk_mapping dmz_alloc_zone !dzone error\n");
		...
		if (l_e) {
			end1 = ktime_get();
			actual_time = ktime_to_ns(ktime_sub(end1, start1));
			trace_printk("[LISTEMPTY] time %lld \n", (long long)actual_time);
		}
		
		dmz_map_zone(zmd, dzone, chunk, 1);
		...
		if (!dmz_is_seq(dzone)) {
			...
			if (c == NULL) { trace_printk("[DEBUG] dmz_get_chunk_mapping c == NULL error \n"); }
			if (c->id != chunk) {
				trace_printk("[DEBUG] dmz_get_chunk_mapping c->id != chunk error \n");
				dzone = ERR_PTR(-EIO);
				goto out;
			}
		}
...
//	trace_printk("[TEST] chunk_in_recl_test\n");
	...
	trace_printk("[SNAP2] dzone(%u)->weight %u dmz_chunk_in_zone_reclaim(c) %d\n", 
					dzone->id, dzone->weight, dmz_chunk_in_zone_reclaim(c));
	while (c->id == chunk && dzone->weight == DMZ_BLOCK_PER_ZONE - 1 && dmz_chunk_in_zone_reclaim(c)) {
		dmz_unlock_map(zmd);
		dmz_unlock_metadata(zmd);
...
chunk_again:
	if (c->id == chunk && dmz_chunk_in_reclaim(c)) {
...
//		dmz_wait_for_reclaim(zmd, dzone);
	...
	if (in_recl) {
		end2 = ktime_get();
		actual_time = ktime_to_ns(ktime_sub(end2, start2));
		trace_printk("[INRECL] time %lld \n", (long long)actual_time);
	}
	/* chunk list modi */
	...
	/*
	struct dm_chunk *c2;
	list_for_each_entry(c2, &zmd->mapped_chunk_list, map_link) {
		if (c2->id == chunk) { 
			dmz_activate_chunk(c2);
			break; 
		}
	}
	*/
	...
	/* chunk list modi */
	if (dzone == NULL) { trace_printk("[DEBUG3] dmz_get_chunk_mapping dzone == NULL\n"); }
	dmz_lru_zone(zmd, dzone, chunk);
	/* chunk list modi */
	if (c->id == chunk) {
		dmz_lru_chunk(zmd, c);
	}
 ...
 * to do it.
 */
void dmz_put_chunk_mapping(struct dmz_metadata *zmd, struct dm_zone *dzone, /*modi*/ unsigned int chunk_id)
{
	struct dm_zone *bzone = NULL;

	dmz_lock_map(zmd);

	if (dmz_is_seq(dzone)) {	// dzone is seq, bzone is buf
		bzone = dzone->bzone;
		if (bzone) {	
			if (dmz_weight(bzone)) {
				dmz_lru_zone(zmd, bzone, chunk_id);
				/* chunk list modi */
			    struct dm_chunk *cc;
			    list_for_each_entry(cc, &zmd->mapped_chunk_list, map_link) {
			        if (cc->id == chunk_id) { 
						break; 
					}
			    }
				...
				if (cc->id == chunk_id) {
					dmz_lru_chunk(zmd, cc);
				}
				else {
					trace_printk("[DEBUGNEWR] dmz_put_chunk_mapping cc->id %u != chunk_id %u dzone %u bzone %u\n",
									cc->id, chunk_id, dzone->id, bzone->id);
				}
				/* chunk list modi */
			}
			else {
				/* Empty buffer zone: reclaim it */
				dmz_unmap_zone(zmd, bzone, chunk_id, -1);
	...
	if (dzone == NULL) {
		trace_printk("[DEBUG] dmz_put_chunk_mapping dzone == NULL error\n");
	}
    ...
    list_for_each_entry(c, &zmd->mapped_chunk_list, map_link) {
        if (c->id == chunk_id) { 
			...
			//trace_printk("[DEBUGCREF] dmz_deactivate_chunk1 c %u refcount %u\n", c->id, atomic_read(&c->refcount));
			break; 
		}
    }
	...
	if (dmz_is_active(dzone) || bzone || dmz_weight(dzone)) {	
		dmz_lru_zone(zmd, dzone, chunk_id);
		/* chunk list modi */
		if (c->id == chunk_id) {
			dmz_lru_chunk(zmd, c);
		}
		...
		if (bzone) { 
			dmz_unmap_zone(zmd, dzone, chunk_id, bzone->id); 
		}
		else { 
			dmz_unmap_zone(zmd, dzone, chunk_id, -1); 
		}
...
struct dm_zone *dmz_get_chunk_buffer(struct dmz_metadata *zmd,
				     struct dm_zone *dzone, /*modi*/ unsigned int chunk_id, unsigned int nr_blocks)
{
...
again:
	...
	dmz_set_chunk_mapping(zmd, chunk_id, dzone->id, bzone->id);
	...
	/* rz chunk weight modi */
	/* chunk list modi */
	set_bit(DMZ_CHUNK_NORMAL, &cc->flags);
	/* chunk list modi */
	INIT_LIST_HEAD(&cc->link);
	/* chunk list modi */
	INIT_LIST_HEAD(&cc->map_link);
	/* chunk list modi */

	/* chunk snapshot modi */
	
	spin_lock_init(&cc->snap_lock);
	...
	while (i < DMZ_BLOCK_PER_ZONE) {
		cc->write_check[i] = -1;
		cc->offsets[i] = -1;
		//trace_printk("[WC] c %u offsets[i] %d set\n", cc->id, cc->offsets[i]);
		i++;
	}
...
//	bzone->bzone = dzone;
	...
	if (dmz_is_cache(bzone)) {
		...
		/* rz chunk weight modi */
		if (bzone->nr_mapped_chunk >= DMZ_CHUNK_PER_RZ) {
		/* rz chunk weight modi */
			list_add_tail(&bzone->link, &zmd->map_cache_list);
		}
		else {
			list_add_tail(&bzone->link, &zmd->unmap_cache_list);
		}
	}
	else
		list_add_tail(&bzone->link, &bzone->dev->map_rnd_list);
			       ...
			       unsigned long flags)
{
	...
	struct dm_zone *zone, *min_wz = NULL;
	int is_alloc_cache = 0;
	int i;
...
//	list_del_init(&zone->link);
	...
	if (is_alloc_cache) {
		list_for_each_entry(zone, list, link) {
			if (!min_wz || zone->weight <= min_wz->weight) {
				if (!min_wz || zone->weight == min_wz->weight && zone->nr_mapped_chunk < min_wz->nr_mapped_chunk) {
					min_wz = zone;
				}
				else if (!min_wz || zone->weight < min_wz->weight) {
					min_wz = zone;
				}
			}
		}
...
//	list_del(&zone->link);
	if (!dmz_is_seq(zone)) {
		zone->nr_mapped_chunk++;
	}
	list_del_init(&zone->link);
	/*
	if (dmz_is_cache(zone) && (zone->nr_mapped_chunk < DMZ_CHUNK_PER_RZ)) {
		list_add_tail(&zone->link, list);
		atomic_inc(&zmd->unmap_nr_cache);
	}
	*/
	/*modi*/
...
void dmz_free_zone(struct dmz_metadata *zmd, struct dm_zone *zone)
{
	...
	if (dmz_is_cache(zone)) {
		//zone->weight = 0;
		//zone->free_block = zmd->zone_nr_blocks;
		list_add_tail(&zone->link, &zmd->unmap_cache_list);
		atomic_inc(&zmd->unmap_nr_cache);
	} else if (dmz_is_rnd(zone)) {
...
void dmz_map_zone(struct dmz_metadata *zmd, struct dm_zone *dzone,
		  unsigned int chunk, /*modi*/ int new_chunk)
{
...
//	dzone->chunk = chunk;
	...
	if (new_chunk) {
		...
		INIT_LIST_HEAD(&cc->map_link);
		/* chunk list modi */
		list_add_tail(&cc->link, &dzone->chunks);
		/* chunk list modi */
		list_add_tail(&cc->map_link, &zmd->mapped_chunk_list);
		/* chunk list modi */
...
void dmz_map_seq_zone(struct dmz_metadata *zmd, struct dm_zone *dzone,
		  unsigned int chunk)
{
	/* Set the chunk mapping */
	dmz_set_chunk_mapping(zmd, chunk, dzone->id,
...
//	if (new_chunk) {
//		struct dm_chunk* cc;
//		cc = kmalloc(sizeof(*dm_chunk), GFP_KERNEL);
//		cc->id = chunk;
//		cc->weight = 0;
//		for (int i = 0; i < DMZ_BLOCK_PER_ZONE; i++) {
//			cc->offsets[i] = -1;
//		}
//		list_add_tail(&cc->link, &dzone->chunks);
//	}
 ...
 * This must be called with the mapping lock held.
 */
void dmz_unmap_zone(struct dmz_metadata *zmd, struct dm_zone *zone /*modi*/, unsigned int chunk_id, int szone_id)
{
//	unsigned int chunk = zone->chunk;
	int dzone_id;

	if (chunk_id == DMZ_MAP_UNMAPPED) {
		/* Already unmapped */
		trace_printk("[DEBUG] dmz_unmap_zone chunk_id == DMZ_MAP_UNMAPPED return\n");
		return;
	}
	...
	if (test_and_clear_bit(DMZ_BUF, &zone->flags)) {
		...
		if (c->id != chunk_id) { 
			trace_printk("[DEBUG] dmz_unmap_zone list_for_each_entry end c->id != chunk_id error\n");
		}
...
//		list_del(&c->link);
	...
	dmz_set_chunk_mapping(zmd, chunk_id, dzone_id, DMZ_MAP_UNMAPPED);
...
//	sector_t chunk_block = 0;
	...
	while (i < DMZ_BLOCK_PER_ZONE) {
		...
		if (IS_ERR(from_mblk)) {
			trace_printk("[DEBUGRECL] dmz_copy_valid_blocks IS_ERR(from_mblk) error\n");
			return PTR_ERR(from_mblk);
		}
		to_mblk = dmz_get_bitmap(zmd, to_zone, i);
		if (IS_ERR(to_mblk)) {
			dmz_release_mblock(zmd, from_mblk);
			trace_printk("[DEBUGRECL] dmz_copy_valid_blocks IS_ERR(to_mblk) error\n");
			return PTR_ERR(to_mblk);
		}

		count = dmz_set_bits((unsigned long *)to_mblk->data, i & DMZ_BLOCK_MASK_BITS, 1);
		if (count) {
		...
		count = dmz_clear_bits((unsigned long *)from_mblk->data, rz_offset & DMZ_BLOCK_MASK_BITS, 1);
		if (count) {
			dmz_dirty_mblock(zmd, from_mblk);
			from_zone->free_block++;
			from_zone->weight--;
		}
...
int dmz_copy_valid_blocks_for_zone_reclaim(struct dmz_metadata *zmd, struct dm_zone *from_zone,
			  struct dm_zone *to_zone, /*modi*/ struct dm_chunk *snapshot, struct dm_chunk *c)
{
	struct dmz_mblock *from_mblk, *to_mblk;
//	struct dm_chunk *c = NULL;
//	sector_t chunk_block = 0;
	...
	list_for_each_entry(c, &from_zone->chunks, link) {
		if (c->id == snapshot->id) { break; }
	}
	if (c == NULL || c->id != snapshot->id) {
		trace_printk("[DEBUGRECL] dmz_copy_valid_blocks c == NULL error\n");
	}
	*/
	/* Get the zones bitmap blocks */
	int i = 0;
	while (i < DMZ_BLOCK_PER_ZONE) {
		if (snapshot->write_check[i] != 1) { i++; continue; }	// no recl
		if (c->offsets[i] != -1) 		   { i++; continue; }	// overwrite
		/* small chunk modi */
		int rz_offset = snapshot->offsets[i];
		/* small chunk modi */
		if (rz_offset == -1) { 									// err
			trace_printk("[WC] c %u snap %u i %d rz_offset == -1 error\n", c->id, snapshot->id, i);
			i++; 
			continue; 
		}
		/*
		if (c->write_check[i] == c->id) {						// overwrite
			to_mblk = dmz_get_bitmap(zmd, to_zone, i);
			if (IS_ERR(to_mblk)) {
				return PTR_ERR(to_mblk);
			}
			count = dmz_clear_bits((unsigned long *)to_mblk->data, i & DMZ_BLOCK_MASK_BITS, 1);
			if (count) {
				dmz_dirty_mblock(zmd, to_mblk);
				to_zone->weight--;
				to_zone->free_block++;
...
//			return ret;
		...
		nr_blocks = ret;
		trace_printk("[DEBUGINV] dmz_merge_valid_blocks dmz_validate_blocks start chunk_id: %u, zone_id: %u\n", c->id, to_zone);
		ret = dmz_validate_blocks(zmd, to_zone, chunk_block, nr_blocks, c->id, 0);
		trace_printk("[DEBUGINV] dmz_merge_valid_blocks dmz_validate_blocks start chunk_id: %u, zone_id: %u\n", c->id, to_zone);
		if (ret)
			return ret;
...
int dmz_merge_valid_blocks_for_zone_reclaim(struct dmz_metadata *zmd, struct dm_zone *from_zone,
			   struct dm_chunk *snapshot, struct dm_zone *to_zone, sector_t chunk_block, struct dm_chunk *c)
{
	unsigned int nr_blocks;
	int ret;
//	struct dm_chunk *c = NULL;
	...
	*/
	...
	while (chunk_block < zmd->zone_nr_blocks) {
		...
		while (i < DMZ_BLOCK_PER_ZONE) {
			if (snapshot->write_check[i] != 1) { i++; continue; } // no recl
			if (c->write_check[i] == c->id) { i++; continue; }
...
//			return ret;
		...
		nr_blocks = ret;
		trace_printk("[DEBUGINV] dmz_merge_valid_blocks dmz_validate_blocks start chunk_id: %u, zone_id: %u\n", snapshot->id, to_zone);
		ret = dmz_validate_blocks(zmd, to_zone, chunk_block, nr_blocks, snapshot->id, 0);
		trace_printk("[DEBUGINV] dmz_merge_valid_blocks dmz_validate_blocks start chunk_id: %u, zone_id: %u\n", snapshot->id, to_zone);
		if (ret)
			return ret;
...
//			return ret;
		...
		trace_printk("[DEBUGINV] dmz_merge_valid_blocks_seq dmz_validate_blocks start chunk_id: %u, zone_id: %u\n", c->id, to_zone->id);
		ret = dmz_validate_blocks(zmd, to_zone, start_block, nr_blocks, c->id, to_zone->wp_block);
		trace_printk("[DEBUGINV] dmz_merge_valid_blocks_seq dmz_validate_blocks end\n");
		//to_zone->wp_block += nr_blocks;
		if (ret)
			return ret;
...
int dmz_merge_valid_blocks_seq_for_zone_reclaim(struct dmz_metadata *zmd, struct dm_zone *from_zone,
			   struct dm_zone *seq_zone, struct dm_chunk *snapshot, struct dm_zone *to_zone, sector_t chunk_block, struct dm_chunk *c)
{
	unsigned int nr_blocks, start_block;
	int ret;
	/*
	...
	*/
	...
	while (chunk_block < zmd->zone_nr_blocks) {
		...
		int i = chunk_block;
		while (i < zmd->zone_nr_blocks) {
			if (c->write_check[i] != 1) { i++; continue; } // no recl
			if (snapshot->write_check[i] != -1) { cur_rz_offset = snapshot->offsets[i]; break; }
			i++;
		}
//		cur_rz_offset = c->offsets[i];
		if (cur_rz_offset == -1 && seq_cont_num <= 0) { break; }
...
//			return ret;
		...
		nr_blocks = ret;
		start_block = chunk_block;
		trace_printk("[DEBUGINV] dmz_merge_valid_blocks_seq dmz_validate_blocks start chunk_id: %u, zone_id: %u\n", snapshot->id, to_zone->id);
		ret = dmz_validate_blocks(zmd, to_zone, start_block, nr_blocks, snapshot->id, to_zone->wp_block);
		trace_printk("[DEBUGINV] dmz_merge_valid_blocks_seq dmz_validate_blocks end\n");
		//to_zone->wp_block += nr_blocks;
		if (ret)
			return ret;
				 ...
				 int set)
{
	...
	if (chunk_block + nr_blocks > zmd->zone_nr_blocks) {
		trace_printk("[DEBUGERR] dmz_to_next_set_block warn_on\n");
	}
	...
	while (nr_blocks) {
		...
		if (IS_ERR(mblk)) {
			trace_printk("[DEBUGERR] dmz_to_next_set_block is_err\n");
			return PTR_ERR(mblk);
		}
...
static int dmz_free_len_in_zone(struct dmz_metadata *zmd, struct dm_zone *zone, unsigned int nr_blocks) {
	sector_t start_block = 0;
	int ret, len;

	while(start_block < zmd->zone_nr_blocks) {
		...
		len = dmz_to_next_set_block(zmd, zone, start_block,
						zmd->zone_nr_blocks - start_block, 1);

		if (nr_blocks <= len) return 1;
...
struct dm_zone *dmz_alloc_zone_len(struct dmz_metadata *zmd, unsigned int dev_idx,
			       unsigned long flags, unsigned int nr_blocks)
{
	struct list_head *list;
	struct dm_zone *zone;
	int i;
...
again:
	if (flags & DMZ_ALLOC_CACHE) {
		...
		list = &zmd->dev[dev_idx].unmap_seq_list;
		trace_printk("[DEBUG] dmz_alloc_zone list == seq\n");
	}

	if (list_empty(list)) {
		trace_printk("[DEBUG] dmz_alloc_zone list_empty error1\n");
		/*
		 * No free zone: return NULL if this is for not reclaim.
		 */
		if (!(flags & DMZ_ALLOC_RECLAIM)) {
			trace_printk("[DEBUG] dmz_alloc_zone list_empty error2\n");
			return NULL;
		}
		/*
		 * Try to allocate from other devices
...
//	list_del(&zone->link);
	list_for_each_entry(zone, list, link) {
		if (dmz_free_len_in_zone(zmd, zone, nr_blocks) > 0) {
			zone->nr_mapped_chunk++;
			list_del_init(&zone->link);
			break;
		}
	...
	if (!zone) {
		trace_printk("[WRITELEN] zone alloc fail error\n");
		return NULL;
	}
	/*
	if (dmz_is_cache(zone) && (zone->nr_mapped_chunk < DMZ_CHUNK_PER_RZ)) {
...
void dmz_set_cache_wp(struct dmz_metadata *zmd, struct dm_zone *rzone, unsigned int nr_blocks) {
	...
	int ret;
	ret = dmz_to_next_set_block(zmd, rzone, start_block, zmd->zone_nr_blocks - rzone->wp_block, 0);

	if (ret == zmd->zone_nr_blocks) {
		trace_printk("[ZONEWP] zone %u full\n", rzone->id);
		trace_printk("[SNAP2] zone %u full\n", rzone->id);
		struct dm_chunk *c;
		list_for_each_entry(c, &rzone->chunks, link) {
			trace_printk("[ZONEWP] chunk %u weight %u\n", c->id, c->weight);
		}
	}
...
void dmz_set_cache_wp_modi(struct dmz_metadata *zmd, struct dm_zone *rzone, unsigned int nr_blocks) {
	sector_t start_block = rzone->wp_block, end_point;
	int ret, len;
	while (start_block < zmd->zone_nr_blocks) {
		ret = dmz_to_next_set_block(zmd, rzone, start_block, zmd->zone_nr_blocks - rzone->wp_block, 0);

		if (ret < 0) {
			trace_printk("[WRITELEN] set_cache_wp fail ERR\n");
			return;
		}

		start_block += ret;

		len = dmz_to_next_set_block(zmd, rzone, start_block, zmd->zone_nr_blocks - rzone->wp_block, 1);
		trace_printk("[WRITELEN] first zone %u start %llu len %llu nr_blocks %u\n", rzone->id, start_block, len, nr_blocks);

		if (nr_blocks <= len) {
			rzone->wp_block = start_block;
			return;
		}
		start_block += len;
	}
	...
	start_block = 0;
	end_point = rzone->wp_block;
	while (start_block < end_point) {
		ret = dmz_to_next_set_block(zmd, rzone, start_block, zmd->zone_nr_blocks, 0);
		...
		len = dmz_to_next_set_block(zmd, rzone, start_block, zmd->zone_nr_blocks, 1);
		trace_printk("[WRITELEN] second zone %u start %llu len %llu nr_blocks %u\n", rzone->id, start_block, len, nr_blocks);
	...
	trace_printk("[WRITELEN] set_cache_wp fail ERR2 zone %u weight %u nr_blocks %u\n", rzone->id, rzone->weight, nr_blocks);
}
/*
 * Validate all the blocks in the range [block..block+nr_blocks-1].
 */
int dmz_validate_blocks(struct dmz_metadata *zmd, struct dm_zone *zone,
			sector_t chunk_block, unsigned int nr_blocks, /*modi*/ unsigned int chunk, unsigned int wp)
{
	...
	if (!dmz_is_seq(zone)) {
		...
		if (c->id != chunk) {
			trace_printk("[DEBUG7] dmz_validate_blocks zone %u c->id %u != chunk %u error\n", zone->id, c->id, chunk);
		}

		while (nr_blocks) {
			/* Get bitmap block */
			mblk = dmz_get_bitmap(zmd, zone, wp);
			if (IS_ERR(mblk))
				return PTR_ERR(mblk);

			/* Set bits */
//			dmz_set_cache_wp(zmd, zone);
			bit = wp & DMZ_BLOCK_MASK_BITS;
			nr_bits = 1;
...
//			spin_lock(&c->snap_lock);
			...
			trace_printk("[WC] c %u chunk_block %u wp %u set\n", c->id, chunk_block, wp);
			/* small chunk modi */
			/* chunk snapshot modi */
//			spin_unlock(&c->snap_lock);
			/* chunk snapshot modi */
...
//		spin_unlock(&c->snap_lock);
	...
	if (likely(zone->weight + n <= zone_nr_blocks)) {
		trace_printk("[DEBUGINVV] dmz_validate_blocks zone %u weight(%u) + n(%u)\n", zone->id, zone->weight, n);
		zone->weight += n;
		zone->free_block -= n;
	}
	else {
		trace_printk("[DEBUGINVV] dmz_validate_blocks zone %u weight %u error n %u\n", zone->id, zone->weight, n);
		trace_printk("[DEBUGINV] dmz_validate_blocks weight error zone %u\n", zone->id);
              ...
              sector_t chunk_block, unsigned int nr_blocks)
{
	...
	trace_printk("[DEBUGINVV] dmz_invalidate_blocks zone %u nr_blocks %u\n", zone->id, nr_blocks);
    ...
    if (zone->weight >= n) {
		trace_printk("[DEBUGINVV] dmz_invalidate_blocks zone %u weight(%u) - n(%u)\n", zone->id, zone->weight, n);
        zone->weight -= n;
		zone->free_block += n;
	}
    else {
		trace_printk("[DEBUGINVV] dmz_invalidate_blocks zone %u weight %u error n %u\n", zone->id, zone->weight, n);
		trace_printk("[DEBUGINV] dmz_invalidate_blocks weight error zone %u\n", zone->id);
        dmz_zmd_warn(zmd, "Zone %u: weight %u should be >= %u",
                 zone->id, zone->weight, n);
        zone->weight = 0;
		zone->free_block += zmd->zone_nr_blocks;
			  ...
			  sector_t chunk_block, unsigned int nr_blocks, /*modi*/ unsigned int chunk_id)
{
	...
	if (dmz_is_seq(zone)) {
		...
		if (c->id != chunk_id) {
			trace_printk("[DEBUG] dmz_invalidate_blocks_modi c->id != chunk_id error\n");
		}
		...
		while (nr_blocks) {
			...
			bit = rzone_off & DMZ_BLOCK_MASK_BITS;
			nr_bits = 1;

			count = dmz_clear_bits((unsigned long *)mblk->data,
							bit, nr_bits);
			if (count) {
				trace_printk("[DEBUGINVV] dmz_invalidate_blocks_modi count %u\n", count);
				dmz_dirty_mblock(zmd, mblk);
				n += count;
			}
			else {
				trace_printk("[DEBUGINVV] already clear\n");
			}
			dmz_release_mblock(zmd, mblk);
	...
	if (zone->weight >= n) {
		trace_printk("[DEBUGINVV] dmz_invalidate_blocks_modi zone %u weight(%u) - n(%u)\n", zone->id, zone->weight, n);
		zone->weight -= n;
		zone->free_block += n;
	}
	else {
		trace_printk("[DEBUGINVV] dmz_invalidate_blocks_modi zone %u wegith %u error n %u\n", zone->id, zone->weight, n);
		trace_printk("[DEBUGINV] dmz_invalidate_blocks_modi weight error zone %u\n", zone->id);
		dmz_zmd_warn(zmd, "Zone %u: weight %u should be >= %u",
			     zone->id, zone->weight, n);
		zone->weight = 0;
		zone->free_block = zmd->zone_nr_blocks;
...
int dmz_invalidate_blocks_modi_for_zone_reclaim(struct dmz_metadata *zmd, struct dm_zone *zone,
			  sector_t chunk_block, unsigned int nr_blocks, /*modi*/ struct dm_chunk *snapshot, struct dm_chunk *c)
{
	unsigned int count, bit, nr_bits;
	struct dmz_mblock *mblk;
	unsigned int n = 0;
	...
	*/
	//int i = chunk_block;
	while (nr_blocks) {
		if (snapshot->write_check[chunk_block] == -1) { nr_blocks--; chunk_block++; continue; } // no recl
		if (c->write_check[chunk_block] == c->id)	  { nr_blocks--; chunk_block++; continue; } // overwrite

		int rzone_off = snapshot->offsets[chunk_block];
		mblk = dmz_get_bitmap(zmd, zone, rzone_off);
		if (IS_ERR(mblk))
			return PTR_ERR(mblk);
		     ...
		     const char *devname)
{
	...
	/* chunk list modi */
	INIT_LIST_HEAD(&zmd->mapped_chunk_list);
	/* chunk list modi */
...
int dmz_resume_metadata(struct dmz_metadata *zmd)
{
	...
	for (i = 0; i < zmd->nr_zones; i++) {
		...
		if (!dmz_is_seq(zone))
			...
			trace_printk("[DEBUGINV] dmz_resume_metadata dmz_invalidate_blocks start zone: %u\n", zone->id);
			dmz_invalidate_blocks(zmd, zone, zone->wp_block,
					      zmd->zone_nr_blocks - zone->wp_block);
			trace_printk("[DEBUGINV] dmz_resume_metadata dmz_invalidate_blocks end\n");
		}
	}
