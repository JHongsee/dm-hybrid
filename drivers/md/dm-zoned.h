...
#define DMZ_ZONE_RECLAIM_MAX_BLOCK	6000
...
enum {
	DMZ_CHUNK_NORMAL,
	DMZ_CHUNK_RECLAIM,
	DMZ_CHUNK_RECLAIM_TERMINATE,
	DMZ_CHUNK_ZONE_RECLAIM,
};

#define dmz_chunk_is_normal(c)	test_bit(DMZ_CHUNK_NORMAL, &(c)->flags)
#define dmz_chunk_in_reclaim(c)	test_bit(DMZ_CHUNK_RECLAIM, &(c)->flags)
#define dmz_chunk_reclaim_should_terminate(c) \
				test_bit(DMZ_CHUNK_RECLAIM_TERMINATE, &(c)->flags)
/* chunk IO block modi */
#define dmz_chunk_in_zone_reclaim(c)	test_bit(DMZ_CHUNK_ZONE_RECLAIM, &(c)->flags)
/* chunk IO block modi */
...
void dmz_map_zone(struct dmz_metadata *zmd, struct dm_zone *zone,
		  unsigned int chunk, int new_chunk);
void dmz_map_seq_zone(struct dmz_metadata *zmd, struct dm_zone *zone,
		  unsigned int chunk);
void dmz_unmap_zone(struct dmz_metadata *zmd, struct dm_zone *zone, unsigned int chunk_id, int szone_id);
...
/* chunk list modi */
static inline void dmz_activate_chunk(struct dm_chunk *c)
{
...
bool dmz_get_high_weight_zone_for_reclaim(struct dmz_metadata *zmd,
					unsigned int dev_idx, bool idle, struct dm_zone *zone, struct dm_chunk **ret_c);
...
struct dm_zone *dmz_get_chunk_mapping(struct dmz_metadata *zmd,
				      unsigned int chunk, int op, unsigned int nr_blocks);
...
/* chunk list modi */
static inline void dmz_deactivate_chunk(struct dm_chunk *c)
{
...
static inline bool dmz_is_active_chunk(struct dm_chunk *chunk)
{
	return atomic_read(&chunk->refcount);
}
